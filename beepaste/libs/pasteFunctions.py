...
from wtforms import TextField, SelectField, validators, RadioField
...
class createPasteForm(BaseForm):
    ...
    pasteLanguage = SelectField(
                        'Syntax Highlighting',
                        choices=languagesList,
                        default='text',
                        validators=[validators.Required()])
    pasteExpire = SelectField(
            'Expire On',
            choices=expireTimes,
            validators=[validators.Required()])
...
def pasteExists(uri, request):
    old_uris_count = request.dbsession.query(Pastes).\
            filter_by(pasteURI=uri).count()
    if old_uris_count > 0:
        return True
    return False
...
def verifyEncryption(enc):
    try:
        count = len([v for i, v in enumerate(
            encryptionMethods) if v[0] == enc])
        if count == 0:
...
def createPaste(form, request):
    ...
    if form.pasteExpire.data != "0":
        ...
        newPaste.expire = datetime.datetime.utcnow() + datetime.timedelta(
                seconds=int(form.pasteExpire.data))
    newPaste.encryption = form.pasteEncryption.data
    newPaste.shortURL = generateShortURL(
            request.route_url('view_paste', pasteID=URI), request)
...
def createPasteFromData(data, request):
    ...
    if data['pasteExpire'] != "0":
        newPaste.toexpire = True
        newPaste.expire = datetime.datetime.utcnow() + datetime.timedelta(
                seconds=int(data['pasteExpire']))
    newPaste.encryption = data['pasteEncryption']
    newPaste.shortURL = generateShortURL(request.route_url(
                                        'view_paste', pasteID=URI),
                                        request)
    request.dbsession.add(newPaste)
    return URI
